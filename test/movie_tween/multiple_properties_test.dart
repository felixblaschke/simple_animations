...
void main() {
  test('multi props - simple', () {
    ...
    expect(
      movie.transform(0.0 / 1.0).get<Color>('color'),
      const Color(0xFF000000),
    );
    expect(
      movie.transform(0.5 / 1.0).get<Color>('color'),
      const Color.from(alpha: 1.0, red: 0.5, green: 0.0, blue: 0.0),
    );
    expect(
      movie.transform(1.0 / 1.0).get<Color>('color'),
  ...
  test('multi props - cascaded', () {
    ...
    expect(
      movie.transform(0.0 / 4.0).get<Color>('color'),
      const Color(0xFF000000),
    );
    expect(
      movie.transform(1.0 / 4.0).get<Color>('color'),
      const Color(0xFF000000),
    );
    expect(
      movie.transform(2.0 / 4.0).get<Color>('color'),
      const Color(0xFF000000),
    );
    expect(
      movie.transform(3.0 / 4.0).get<Color>('color'),
      const Color.from(alpha: 1.0, red: 0.5, green: 0.0, blue: 0.0),
    );
    expect(
      movie.transform(4.0 / 4.0).get<Color>('color'),
      const Color(0xFFFF0000),
    );
  });
}
